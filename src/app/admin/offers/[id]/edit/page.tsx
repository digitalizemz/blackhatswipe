...
export default function EditOfferPage() {
  ...
  useEffect(() => {
    ...
    async function fetchOffer() {
      const { data, error: fetchErr } = await supabase.from('offers').select('*').eq('id', id).single()
      if (fetchErr || !data) { setNotFound(true); setFetching(false); return }

      setTitle(data.title ?? '')
      setNicheId(data.niche_id ?? '')
  ...
  if (notFound) {
    return (
      <div className="p-8">
        <p className="text-red-400 text-sm mb-2">Offer not found.</p>
        <Link href="/admin/offers" className="text-zinc-400 hover:text-white text-sm">← Back to Offers</Link>
      </div>
    )
  }
      ...
      <div className="p-8">
        ...
        <div className="flex items-center gap-3 mb-8 max-w-3xl mx-auto">
          ...
          <div
            ...
            onClick={() => router.push(`/admin/offers/${id}/materials`)}
          >
            ...
            <span className="text-sm text-zinc-400 ml-2">Materials</span>
          </div>
        </div>
